...
get_env_specific_direct_dependencies () {
	...
	echo "$DIRECT_DEPS" \
	| awk '/\/opt\/homebrew\//,/.dylib/ {print $1}'
}
...
get_env_specific_dependencies_recursive () {
	while read -r obj; do
		[ -z "$obj" ] && continue
    echo "$obj"
		get_env_specific_direct_dependencies "$obj"
...
mv "$LIB/libgraphite2.3.2.1.dylib" "$LIB/libgraphite2.3.dylib"
mv "$LIB/libpixman-1.0.42.2.dylib" "$LIB/libpixman-1.0.dylib"
mv "$LIB/libxcb-shm.0.0.0.dylib" "$LIB/libxcb-shm.0.dylib"
mv "$LIB/libxcb-render.0.0.0.dylib" "$LIB/libxcb-render.0.dylib"
mv "$LIB/libxcb.1.1.0.dylib" "$LIB/libxcb.1.dylib"
mv "$LIB/libjpeg.8.2.2.dylib" "$LIB/libjpeg.8.dylib"

ls "$LIB" | xargs -I'{}' chmod +w "$LIB/{}"
