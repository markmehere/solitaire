...
extern void addStockTranslation(const char *stock_id, const char *label);
...
ar_stock_init (void)
{
  ...
  for (int i = 0; i < G_N_ELEMENTS(ar_stock_items); i++) {
    addStockTranslation(ar_stock_items[i].stock_id, ar_stock_items[i].label);
  }
