...
#define SCHEMA_NAME           "org.pazolli.Solitaire.Preferences"
#define AR_SETTINGS_PREFERENCES_PATH "/org/pazolli/solitaire/preferences/"
...
#else
  conf_dir = g_build_filename (getApplicationSupportPath (), NULL, NULL);
#endif
  if (!conf_dir)
    goto loser;
...
ar_conf_init (ArConf *conf)
{
  ...
  priv->settings = g_settings_new_with_path (SCHEMA_NAME, AR_SETTINGS_PREFERENCES_PATH);
			  ...
			  GObjectConstructParam *construct_params)
{
  ...
  conf_file = g_build_filename (getApplicationSupportPath (), game_name, NULL);
                        ...
                        GError ** error)
{
  ...
  if (group) {
    ...
    return g_settings_get_boolean(priv->settings, key);
  }
#endif /* HAVE_GNOME */
}
                        ...
                        gboolean value)
{
  ...
  if (group) {
    ...
    g_settings_set_boolean(priv->settings, key, value);
  }
  g_signal_emit (instance, signals[VALUE_CHANGED], 0, group, key);
#endif /* HAVE_GNOME */
}
