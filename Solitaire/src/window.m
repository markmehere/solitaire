...
#include "window.h"
#include "SolMenuBar.h"
#include "SolActionDictionary.h"
#include "SolFinishEffects.h"
...
#define AR_SETTINGS_WINDOW_STATE_PATH "/org/pazolli/solitaire/window-state/"

NSString *curGameModule;

GtkApplication *app;
AisleriotWindow *active_win;
...
static void
toolbar_toggled_cb (GSimpleAction *action,
                    AisleriotWindow *window)
{
  AisleriotWindowPrivate *priv = window->priv;
  gboolean state;
  ...
  GVariant *updatedVariant = g_variant_new_boolean(state);
...
static void
statusbar_toggled_cb (GSimpleAction *action,
                      AisleriotWindow *window)
{
  AisleriotWindowPrivate *priv = window->priv;
  gboolean state;
                        ...
                        gboolean is_fullscreen)
{
  ...
  setItemChecked("Fullscreen", is_fullscreen);
  
}

static void
fullscreen_toggled_cb (GSimpleAction *action,
                       GtkWindow *window)
{
  gboolean state;
  ...
  ar_debug_print (AR_DEBUG_WINDOW_STATE,
                      ...
                      window ? window : GTK_WINDOW (active_win),
                      state ? "going" : "leaving");


  if (state) {
    gtk_window_fullscreen (window ? window : GTK_WINDOW (active_win));
  } else {
    gtk_window_unfullscreen (window ? window : GTK_WINDOW (active_win));
  }
}
...
static void
clickmove_toggle_cb (GSimpleAction *action,
                     AisleriotWindow *window)
{
  AisleriotWindowPrivate *priv = window->priv;
  ...
  GVariant *variant = g_action_get_state(G_ACTION(action));
  click_to_move = !g_variant_get_boolean(variant);
  GVariant *updatedVariant = g_variant_new_boolean(click_to_move);
  g_simple_action_set_state(action, updatedVariant);
  g_variant_unref(variant);
...
static void
option_cb (GSimpleAction *action,
           AisleriotWindow *window)
{
  AisleriotWindowPrivate *priv = window->priv;
  ...
  changed_value = clearAllRadiosInGroup (g_action_get_name (G_ACTION (action)), &changed_mask);
...
install_options_menu (AisleriotWindow *window)
{
  ...
  GList *firstOption = options;
  if (!options) {
    resetOptionsMenu(app);
    return;
  }
  ...
  for (l = options; l != NULL; l = l->next) {
    ...
    if (option->type == AISLERIOT_GAME_OPTION_CHECK) {
      addCheckOption(actionname, option->display_name, G_CALLBACK(option_cb), l == firstOption, option->set, app);
      ...
      addRadioOption(actionname, option->display_name, radion, G_CALLBACK(option_cb), l == firstOption, option->set, app);
                ...
                GVariant *variant)
{
  ...
  game_module = g_variant_get_string(variant, NULL);
  g_return_if_fail (game_module != NULL);

  aisleriot_window_set_game_module (active_win, game_module, NULL);
...
install_recently_played_menu (AisleriotWindow *window)
{
  ...
  for (i = 0; i < n_recent; ++i) {
    ...
    addRecentGame(app, game_name, ar_filename_to_game_module (recent_games[i]), G_CALLBACK (recent_game_cb), i == 0);
/*
    ...
    g_object_unref (action);    gtk_ui_manager_add_ui (priv->ui_manager,
...
*/
  ...
  if (n_recent == 0) {
      addRecentGame(app, "Klondike", ar_filename_to_game_module("Klondike"), G_CALLBACK (recent_game_cb), true);
  }
                 ...
                 AisleriotWindow *window)
{
  ...
  gtk_action_set_sensitive (priv->action[ACTION_HINT],
    ...
    setQuickAccessItemEnabled (ACTION_HINT, state == GAME_BEGIN || state == GAME_RUNNING);
                    ...
                    AisleriotWindow *window)
{
  ...
  setQuickAccessItemEnabled (ACTION_UNDO_MOVE, enabled);
  ...
  setQuickAccessItemEnabled (ACTION_RESTART_GAME, enabled);
}
                    ...
                    AisleriotWindow *window)
{
  ...
  setQuickAccessItemEnabled (ACTION_REDO_MOVE, enabled);
}
                    ...
                    AisleriotWindow *window)
{
  ...
  setQuickAccessItemEnabled (ACTION_DEAL, enabled);
}
...
aisleriot_window_init (AisleriotWindow *window)
{
  const GtkActionEntry actions[] = {
    ...
    { "UndoMove", AR_STOCK_UNDO_MOVE, NULL, "<Meta>Z",
      N_("Undo the last move"),
      G_CALLBACK (undo_cb) },
    { "RedoMove", AR_STOCK_REDO_MOVE, NULL, "<Meta><Shift>Z",
  ...
  for (int i = 0; i < G_N_ELEMENTS (names); i++) {
    addQuickAccessItem (names[i], i == 0);
  }
  ...
  SolActionDictionary *dict = createActionDictionary(40);
  addActions(dict, actions, G_N_ELEMENTS(actions));
  addToggleActions(dict, toggle_actions, G_N_ELEMENTS(toggle_actions));
  createMenuBar(app, dict, window);
  ...
  setItemChecked("Toolbar", priv->toolbar_visible);
  g_object_set (priv->toolbar, "visible", priv->toolbar_visible, NULL);
  action = gtk_action_group_get_action (priv->action_group, "ClickToMove");
  gboolean clickToMove = ar_conf_get_boolean (NULL, aisleriot_conf_get_key (CONF_CLICK_TO_MOVE), NULL);
  setItemChecked("ClickToMove", clickToMove);
    
  aisleriot_game_set_click_to_move (priv->game, clickToMove);
  ar_style_set_click_to_move (priv->board_style, clickToMove);
  ...
  g_object_set (priv->statusbar, "visible", priv->statusbar_visible, NULL);
  setItemChecked("Statusbar", priv->statusbar_visible);
...
{
  app = application;
  active_win = g_object_new (AISLERIOT_TYPE_WINDOW,
                         "application", application,
                         "show-menubar", FALSE,
                         NULL);
  return GTK_WIDGET (active_win);
}
                                  ...
                                  GRand *rand)
{
  ...
  curGameModule = [NSString stringWithUTF8String:game_module];
